...
def random_sample():
    with open(
        "data/hotpot_QA/hotpot_dev_distractor_v1.json", "r", encoding="utf-8"
    ) as f:
        data = json.load(f)
    data = [item for item in data if item["level"] == "hard"]
    random_data = random.sample(data, 200)
    with open(
        "data/hotpot_QA/hotpot_dev_distractor_v1_200.json", "w", encoding="utf-8"
    ) as f:
...
def convert_data():
    with open(
        "data/hotpot_QA/hotpot_dev_distractor_v1_200.json", "r", encoding="utf-8"
    ) as f:
        data = json.load(f)
    ...
    for idx, item in enumerate(data):
        ...
        for context in item["context"]:
            ...
            if conversion_type == "text":
                converted_context.append([context[0], "text", "\n".join(context[1])])
            else:
                prompt = construct_instructions_without_claim[conversion_type].format(
                    evidence_text=f"{context[0]}\n\n" + "\n".join(context[1])
                )
                all_prompts.append(prompt)
                prompt_meta.append((idx, context[0], conversion_type))
    ...
    with open(
        "data/hotpot_QA/hotpot_dev_distractor_v1_200_converted.json",
        "w",
        encoding="utf-8",
    ) as f:
...
def run_qa_evaluation_homo(model_name):
            ...
            input_data=str(
                {
                    ...
                    "context": [
                        context
                        for context in item["context"]
                        if context[0] in [i[0] for i in item["supporting_facts"]]
                    ],
                }
            )
        )
    ...
    with open(
        f"data/hotpot_QA/hotpot_dev_distractor_v1_200_{model_name}_homo_useful_info.json",
        "w",
        encoding="utf-8",
    ) as f:
...
def run_qa_evaluation_hete(model_name):
    ...
    with open(
        f"data/hotpot_QA/hotpot_dev_distractor_v1_200_{model_name}_hete_useful_info.json",
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(llm_answers, f, indent=2)
...
def run_judge(model_name):
    ...
    with open(
        f"data/hotpot_QA/hotpot_dev_distractor_v1_200.json",
        "r",
        encoding="utf-8",
    ) as f:
    ...
    with open(
        f"data/hotpot_QA/judge_{model_name}_useful_info.json", "w", encoding="utf-8"
    ) as f:
...
def run_judge_inject(model_name):
    with open(
        f"data/hotpot_QA/hotpot_dev_distractor_v1_200_{model_name}_useful_info_inject.json",
        "r",
        encoding="utf-8",
    ) as f:
        hete_answers = json.load(f)
    with open(
        f"data/hotpot_QA/hotpot_dev_distractor_v1_200.json", "r", encoding="utf-8"
    ) as f:
        hotpot_data = json.load(f)
        questions = [item["question"] for item in hotpot_data]
        gold_answers = [item["answer"] for item in hotpot_data]
    ...
    with open(
        f"data/hotpot_QA/judge_{model_name}_useful_info_inject.json",
        "w",
        encoding="utf-8",
    ) as f:
        json.dump(all_judgments, f, indent=2)
...
def run_hook(model_path):
    ...
    with open(
        f"data/hotpot_QA/hotpot_dev_distractor_v1_200_converted.json",
        "r",
        encoding="utf-8",
    ) as f:
        data = json.load(f)
