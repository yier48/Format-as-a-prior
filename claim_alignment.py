...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Specific model name to answer (e.g., Llama-3.1-8B-Instruct-FP8)",
    )
    parser.add_argument(
        "--judge_model_name",
        type=str,
    ...
    with open("./data/converted/converted_output.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    with open(f"{model_id}_converted_output.json", "w", encoding="utf-8") as f:
        json.dump(all_answers, f, ensure_ascii=False, indent=2)

    with open(
        f"./data/preference/all_answers/{model_id}_converted_output.json",
        "r",
        encoding="utf-8",
    ) as f:
    ...
    with open(
        f"{model_id}_converted_output_judge_{judge_model}.json", "w", encoding="utf-8"
    ) as f:
